...
def find_authors_nationalities():
    ...
    return '\n'.join((str(author) + f' is {author.nationality}') for author in authors)
...
def order_books_by_year():
    ...
    return '\n'.join((f'{book.publication_year} year: ' + str(book)) for book in books)
...
def delete_review_by_id(review_id: int) -> str:
    # review_to_delete = Review.objects.filter(id=review_id).first()
...
def filter_authors_by_nationalities(nationality: str) -> str:
    ...
    return '\n'.join(author.biography if author.biography else str(author) for author in authors)
...
def filter_authors_by_birth_year(from_year, to_year):
    # from_date = datetime(from_year, 1, 1)
    # to_date = datetime(to_year, 12, 31)
    # authors = Author.objects.filter(birth_date__range=(from_date, to_date)).order_by('-birth_date')
    authors = Author.objects.filter(birth_date__year__range=(from_year, to_year)).order_by('-birth_date')
    return '\n'.join(f'{autor.birth_date}: ' + str(autor) for autor in authors)
